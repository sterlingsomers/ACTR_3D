...
class BottomUpVision(ccm.ProductionSystem):
    ...
    def monitor_ap_retrieve_width(b_vision_command='scan:opening get:motor'):
        motor_module.get_bounding_box()
        motor_module.request_bounding_box()
        motor_module.request('type:proprioception feature:bounding_box width:? depth:?',delay=0.00)
    ...
    def monitor_ap_retrieve_vision(b_vision_command='scan:opening get:vision',
         ...
         vision_module.request('isa:opening',delay=0.00)
         b_motor.clear()
...
class MotorMethods(ccm.ProductionSystem):
    production_time = 0.010
    #fake_buffer = Buffer()
    shoulder_rotation_rate = 11500 * 0.0129**3
...
log=ccm.log(data=True,screen=False,directory="Experiment0_NoMotorInteferenceWalking_nonLinear_lessVisualInterference_AFalternate_modulated_LINEAR_fasterAF/RadiusMultiplier(1.9) MaximumRotationRate(0.08)")
model=MyModel()
