...
class MotorMethods(ccm.ProductionSystem):
    ...
    def increase_rotation_abdomen_left(b_motor_command_abdomen='rotate:true direction:left rate:?r', motor_module='busy:False'):
        print('rrrr',r)
...
###########################
    ...
    def start_experiment_large_opening_found(b_plan_unit='planning_unit:walk_through_aperture',
                                             ...
                                             b_vision1='isa:opening centre:?where width:?w'):
        b_vision_command.set('scan:obstacles get:body_dimensions alert_status:none')
        #goal.set('stop')
        DM.add('isa:opening width:' + w)
        b_plan_unit.clear()
        b_unit_task.clear()
        b_operator.clear()
    ...
    def vision_scan_obstacle_left(b_plan_unit='planning_unit:walk_through_aperture',
        ...
        #vision_module.find_feature(feature='opening_size', depth=0, width=0, delay=0.05)
        #Assumes you already know there is an opening (procedural knowledge)

        DM.request('isa:opening width:?')
        b_operator.modify(operator='plan_width')

    def vision_scan_obstacle_left_plan_width(b_plan_unit='planning_unit:walk_through_aperture',
                                  b_unit_task='unit_task:walk posture:standing',
                                  b_operator='operator:plan_width isa:obstacle location:left',
                                  DMbuffer='width:?w'):
        b_motor_command_shoulders.set('rotate:true direction:left rate:' + w)
        b_motor_command_abdomen.set('rotate:true direction:left rate:' + w)
