...
class BottomUpVision(ccm.ProductionSystem):
    ...
    def detect_obstacles_two_alert(b_vision_command='scan:obstacles get:visual_obstacles alert_status:alert',
        ...
        vision_module.find_feature(feature='obstacle', depth=d, width=w, radius_multiplier=self.parent.RadiusMultiplier, delay=0.02)
...
class MotorMethods(ccm.ProductionSystem):
    ...
    def increase_rotation_abdomen_left(b_motor_command_abdomen='rotate:true direction:left check:false', motor_module='busy:False'):
        ...
        b_motor_command_abdomen.modify(check='box')
        #if self.shoulder_rotation_rate >= self.maximum_rotation_rate:
        #    self.shoulder_rotation_rate = self.maximum_rotation_rate
        #goal.set('stop')

    def increase_rotation_abdomen_right(b_motor_command_abdomen='rotate:true direction:right check:false', motor_module='busy:False'):
    ...
    def check_rotation(b_motor_command_abdomen='rotate:true check:box', motor_module='busy:False'):
        motor_module.get_bounding_box()
        motor_module.request_bounding_box()
        motor_module.request('type:proprioception feature:bounding_box width:? depth:?',delay=0.01)
        b_motor_command_abdomen.modify(check='vision')


    #THIS MIGHT WORK but the vision system is too innacurate
    #seems that width is sometimes 0?
    #The easiest way to get this to work is implement the b2
    def check_rotation2(b_motor_command_abdomen='rotate:true check:vision', motor_module='busy:False', vision_module='busy:False',b_motor='width:?w depth:?d'):
        print('wds',w,d)
        vision_module.find_feature(feature='opening', width=float(w)*VisionMultiplier, depth=d)
        vision_module.request('isa:opening',buffer=2,delay=0.02)
        b_motor_command_abdomen.modify(check='result')

    def check_rotation3(b_motor_command_abdomen='rotate:true check:result', motor_module='busy:False', b_vision2='isa:opening'):
        b_motor_command_abdomen.clear()
        self.parent.b_plan_unit.set('planning_unit:walk_through_aperture')
        self.parent.b_unit_task.set('unit_task:walk posture:standing')
        self.parent.b_operator.set('operator:react isa:obstacle location:' + l)

    def check_rotation3_fail(b_motor_command_abdomen='rotate:true check:result', motor_module='busy:False', vision_module='error:True'):
        b_motor_command_abdomen.modify(check='false')
...
###########################
    ...
    def vision_scan_obstacle_left(b_plan_unit='planning_unit:walk_through_aperture',
        ...
        b_motor_command_abdomen.set('rotate:true direction:left check:false')
        #b_vision_command.set('scan:obstacles get:body_dimensions alert_status:alert')
        #b_plan_unit.clear()
        b_plan_unit.set('planning_unit:walk_through_aperture')
        b_unit_task.set('unit_task:manage_rotation')
    ...
    def vision_scan_obstacle_right(b_plan_unit='planning_unit:walk_through_aperture',
        ...
        b_motor_command_abdomen.set('rotate:true direction:right check:false')
        #b_vision_command.set('scan:obstacles get:body_dimensions alert_status:alert')
        #b_plan_unit.clear()
        b_plan_unit.set('planning_unit:walk_through_aperture')
        b_unit_task.set('unit_task:manage_rotation')
...
log=ccm.log(data=True,screen=False,directory="Experiment0_NoMotorInteferenceWalking_nonLinear_lessVisualInterference_alternate/RadiusMultiplier(1.8)")
model=MyModel()
