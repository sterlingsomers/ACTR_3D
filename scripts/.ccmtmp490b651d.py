...
###########################
    ...
    def vision_scan_obstacle_modulate_left(b_plan_unit='planning_unit:walk_through_aperture',
                                  ...
                                  b_vision1='isa:opening width:?w',
                                  b_motor='width:?mW'):
        #goal.set('stop')

        b_motor_command_shoulders.set('rotate:true direction:left opening:' + w + ' width:' + mW)
        b_motor_command_abdomen.set('rotate:true direction:left opening:' + w + ' width:' + mW)
        b_plan_unit.set('planning_unit:walk_through_aperture')
        b_unit_task.set('unit_task:manage_rotation')
        b_operator.set('operator:retrieve_width')
        #goal.set('stop')
...
log=ccm.log(data=True,screen=False,directory="LinearModel_Planned_LowLevel/RadiusMultiplier(1.9) ApertureWidth(70) WalkingRate(0.0177)")
model=MyModel()
