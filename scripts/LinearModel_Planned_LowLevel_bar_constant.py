...
class MotorMethods(ccm.ProductionSystem):
    ...
    def increase_rotation_abdomen_left(b_motor_command_abdomen='rotate:true direction:left opening:?w width:?mW', motor_module='busy:False'):
        openingWidth = float(w)
        agentWidth = float(mW)
...
class timeKeeper(ccm.Model):
    ...
    def record_data(self,now):
        ...
        if not self.recorded:
            ...
            log.RadiusMultiplier = RadiusMultiplier
            log.VisionMultiplier = VisionMultiplier
            log.VisionConstant = VisionConstant
            log.ApertureWidth = ApertureWidth
            log.AgentWidth = AgentWidth
            log.AgentWithBarWidth = AgentWithBarWidth
            log.APRatio = APRatio
            log.BarRatio = BarRatio
...
class MyModel(ACTR):
    ...
    def setup_zero_stop(goal='setup:zero',b_count='value:10'):
        motor_module.get_bounding_box()
        timeKeep.record_bbox(motor_module._boundingBox)
...
###########################
    ...
    def vision_scan_obstacle_left(b_plan_unit='planning_unit:walk_through_aperture',
        ...
        print("W: ", float(w) + self.VisionMultiplier)
        vision_module.find_feature(feature='opening', width=float(w)*self.VisionMultiplier, depth=d)
        vision_module.request('isa:opening centre:? left:? right:? width:?')
        b_operator.modify(operator='modulate')

    def vision_scan_obstacle_modulate_left(b_plan_unit='planning_unit:walk_through_aperture',
                                  b_unit_task='unit_task:walk posture:standing',
                                  b_operator='operator:modulate isa:obstacle location:left',
                                  vision_module='error:True',
                                  b_motor='width:?mW'):
        #goal.set('stop')

        b_motor_command_shoulders.set('rotate:true direction:left opening:' + '0' + ' width:' + '0')
        b_motor_command_abdomen.set('rotate:true direction:left opening:' + '0' + ' width:' + '0')
        b_plan_unit.set('planning_unit:walk_through_aperture')
        b_unit_task.set('unit_task:manage_rotation')
        b_operator.set('operator:retrieve_width')
        #goal.set('stop')
    ...
    def manage_rotation_check_gap(b_plan_unit='planning_unit:walk_through_aperture',
                                    ...
                                    b_motor='width:?w depth:?d'):
        print("BBWIDTH",w)
        vision_module.find_feature(feature='opening', width=float(w)+self.VisionConstant, depth=d)
        vision_module.request('isa:opening',delay=0.05)
        b_operator.set('operator:check_opening')
        #goal.set('stop')
        #b_plan_unit.clear()
...
model.VisionConstant = VisionConstant
model.middleware = middleware
middleware.send('set_walls',value=ApertureWidth)
