...
class MyAgent(ACTR):
    ...
    def init():
        ...
        DM.add('color_a:blue property:object what:marker')
        DM.add('color_a:red property:object what:apple')
    ...
    def condiment(focus='get_condiment'):
        ...
        DM.request('color_a:blue what:?')                # retrieve a chunk from DM into the DM buffer
        focus.set('sandwich condiment')         # ? means that slot can match any content

    def order(focus='sandwich condiment', DMbuffer='what:?condiment'):  # match to DMbuffer as well
